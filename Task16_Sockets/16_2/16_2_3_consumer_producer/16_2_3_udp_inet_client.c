...
static void PrintErrorStrAndExit(char *err_str, int caller_line) {
  err(EXIT_FAILURE, "[ %9s [%d]: %s: %d ]", k_server_types_str[SERV_T_CLIENT],
      k_server_main_port, err_str, caller_line);
}

static void CheckError(int err_int, char *err_str, int caller_line) {
  if (err_int < 0)
    PrintErrorStrAndExit(err_str, caller_line);
}

static void InteractWithSubServerViaSockFd(int sock_fd) {
  char msg_recv[STR_SIZE_MAX] = {0};
  char msg_send[STR_SIZE_MAX] = {0};

  CheckError(printf("*[%s]: Пробуем подключиться к серверу...*\n", ip_addr_str),
             "printf", __LINE__);
...
static void InputNodeForIp(char *const restrict node_str) {
  int ret_getchar = 0;
...
static int GetSubServerSockFd() {
  ...
  // создаём сокет
  CheckError(sock_fd = socket(local_addr.sin_family, SOCK_DGRAM, 0), "socket",
             __LINE__);
  ...
  CheckError(connect(sock_fd, (struct sockaddr *)&server_addr, len), "connect",
             __LINE__);
