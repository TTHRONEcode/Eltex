...
void ChHeaderDirStr(int p_num, char *p_str_in) {
  ...
  int l_max_head_size = getmaxx(l_loc_header) > 6 ? getmaxx(l_loc_header) - 6
                                                  : getmaxx(l_loc_header);
...
static void RedrawMenu(int k) {
  ...
  wresize(g_menu_wnd[k], g_row[k] - 2, g_col[k] - 3);
...
void SigWinCh(int p_signo) {
  ...
  // werase(g_headers_wnd[1]);
  wrefresh(g_headers_wnd[1]);
  wrefresh(g_headers_wnd[2]);
  // refresh();
}
...
int MenuManager() {
  ...
  while (1) {
    ...
    while (g_need_reset[g_head_n] == 0) {
      ...
      g_input[g_head_n] = wgetch(g_windows_wnd[g_head_n]);
